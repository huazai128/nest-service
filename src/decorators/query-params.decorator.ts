...
export const QueryParams = createParamDecorator((customConfig: TTransformConfig[] , request): IQueryParamsResult => {
    ...
    // 合并配置
    if (customConfig) {
        customConfig.forEach((field: string) => {
    ...
    const validates: IValidateError[] = [
        ...
        },
        {
            ...
            isAllowed: lodash.isUndefined(date) || Array.isArray(date),
            isIllegal: false,
            setValue() {
                if (date != null && !!date.length) {
                    const startTime = new Date(date[0]).getTime()
                    const endTime = new Date(date[1]).getTime()
                    if (!Object.is(startTime.toString(), 'Invalid Date') && !Object.is(endTime.toString(), 'Invalid Date')) {
                        querys.create_at = {
                            $gte: startTime,
                            $lt: endTime,
                        }
    ...
    const result: IQueryParamsResult = {
        querys,
        options,
        params,
